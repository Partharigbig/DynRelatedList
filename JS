...
export default class MedicalRelatedListByRT extends NavigationMixin(LightningElement) {
  ...
  set fieldsToShow(v) {
    ...
    this.buildColumns(!!this.objectInfo); // << was this.objectInfo?.data
  }
  // State
  allRows = [];
  rows = [];
  ...
  objectInfo;        // << keep the data here (not .data)
  canCreate = false;
  canEdit = false;
  ...
  objectInfoWire({ data }) {
    if (data) {
      this.objectInfo = data;           // << store data
      this.canCreate = !!data.createable;
      this.canEdit   = !!data.updateable;
      this.buildColumns(true);          // << rebuild with real labels
    }
  }
  // Computed
  ...
  renderedCallback() {
    if (this.objectInfo && !this._columnsBuiltWithMeta) {   // << was .data
  ...
  buildColumns(useMeta = false) {
    const info = useMeta ? this.objectInfo : null;          // << was this.objectInfo?.data
    const cols = (this.fieldApiNames || []).map((f) => {
      // Prefer real Salesforce field label; otherwise prettify API name
      ...
      // Make Name clickable, but keep dynamic label (e.g., "Medical Conditions")
      if (f === 'Name') {
        // === CUSTOM OVERRIDE: If this widget is showing the Allergy Intolerance record type,
        // rename the Name column to "Allergy Id". Otherwise keep the metadata label.
        const nameLabel = (this.recordTypeDeveloperName === 'Allergy_Intolerance') ? 'Allergy ID' : label;
        return {
          label: nameLabel,
          fieldName: 'recordUrl',
          type: 'url',
